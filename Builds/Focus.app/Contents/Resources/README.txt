...
The authorization right for this operation defaults to allowing anyone to do it, meaning that any user on the system can run the make-believe app controlled by this license key.  However, a system admin can change this default by editing the right specification for the "BradJasper.focus.readLicenseKey" right in the authorization policy database (currently "/etc/authorization").  For example, the system admin could change this right so that only certain users can read the license key and thus run the app.

o Write License -- Click this to ask the privileged helper tool to save the make-believe software license key.

The authorization right for this operation defaults to requiring admin authentication.  However, a system admin can change this default by editing the right specification for the "BradJasper.focus.writeLicenseKey" right in the authorization policy database.
...
The authorization right for this operation defaults to allowing anyone to do it.  Again, a system admin can change this default by editing the right specification in the authorization database, this time for the "BradJasper.focus.startWebService" right.
